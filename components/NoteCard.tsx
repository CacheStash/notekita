    ...
    <div className={`group bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 p-6 flex flex-col transition-all duration-300 hover:shadow-xl ${viewMode === 'list' ? 'flex-row items-center py-4' : 'space-y-4'}`}>
      <div className="flex-1">
        ...
        <div className="relative mt-2">
          ...
          <button onClick={() => {
              if(!showSensitive && (note.isPrivate || user?.isContentHidden)) {
                if (user?.isPinEnabled && user.pin) {
                  const p = window.prompt('Masukkan PIN:');
                  if(p === user.pin) setShowSensitive(true);
                } else setShowSensitive(true);
              } else setShowSensitive(!showSensitive);
            }} 
